...
def parse_task(text:str) -> Optional[Tuple[str, str, datetime, Optional[int], Optional[int], Optional[int]]]:
    ...
    m = re.fullmatch(r"через\s+(\d+)\s+(секунд[уы]?|минут[уы]?|час(?:ов|а)?)\s+(.+)", t)
    if m:
        n = int(m.group(1))
        unit = m.group(2)
        title = m.group(3).strip()
        delta = timedelta(seconds=n) if unit.startswith("сек") else \
                timedelta(minutes=n) if unit.startswith("мин") else \
                timedelta(hours=n)
        run_local = now_tz() + delta
        return ("once", title, run_local, None, None, None)

    # сегодня/завтра в HH:MM
    m = re.fullmatch(r"(сегодня|завтра)\s+в\s+(\d{1,2}):(\d{2})\s+(.+)", t)
    if m:
        day = now_tz().date() + timedelta(days=(1 if m.group(1)=="завтра" else 0))
        hh, mm = int(m.group(2)), int(m.group(3))
        title = m.group(4).strip()
        run_local = datetime.combine(day, time(hh, mm))
        run_local = TZ.localize(run_local)
        return ("once", title, run_local, None, None, None)

    # каждый день в HH:MM
    m = re.fullmatch(r"кажд[ыйе]\s+день\s+в\s+(\d{1,2}):(\d{2})\s+(.+)", t)
    if m:
        hh, mm = int(m.group(1)), int(m.group(2))
        title = m.group(3).strip()
        run_local = now_tz()
        return ("daily", title, run_local, hh, mm, None)

    # 30 августа в 10:00 что-то
    m = re.fullmatch(r"(\d{1,2})\s+([а-я]+)\s+в\s+(\d{1,2}):(\d{2})\s+(.+)", t)
    if m and m.group(2) in months:
        ...
        year = now_tz().year
        run_local = TZ.localize(datetime(year, mon, d, hh, mm))
        return ("once", title, run_local, None, None, None)
...
async def affairs_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_auth(chat):
        ...
        return
    with db() as c:
        rows = c.execute("SELECT id,title,type,run_at_utc,hour,minute,day_of_month FROM tasks WHERE chat_id=? AND active=1 ORDER BY id",
                         (chat,)).fetchall()
    if not rows:
        await update.message.reply_text("Дел нет.")
        return
    lines = []
    for i,(tid,title,typ,run,h,m,dom) in enumerate(rows, start=1):
        if typ=="once" and run:
            lines.append(f"{i}. {title} — {fmt(datetime.fromisoformat(run))}")
        elif typ=="daily":
            lines.append(f"{i}. {title} — каждый день в {h:02d}:{m:02d}")
        else:
            lines.append(f"{i}. {title} — каждый месяц, {dom}-го в {h:02d}:{m:02d}")
...
async def affairs_delete_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    m = re.fullmatch(r"/affairs_delete\s+(\d+)", text)
    if not m:
        await update.message.reply_text("Формат: /affairs_delete N (номер из /affairs)")
        return
    idx = int(m.group(1))
    with db() as c:
        ...
        if idx<1 or idx>len(ids):
            await update.message.reply_text("Неверный номер. Сначала открой /affairs.")
            return
        tid = ids[idx-1]
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    t = get_task(tid)
    try:await schedule(ctx.application, t)
        if t.type == "once":
